...
const starts = async (dha = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('BELAJAR DULU DEK', {
		font: 'Standard',
	...
	console.log(color('[ SOURCE CODE INI DI RECODE OLEH ZAKIR HORIZONTAL\nDENGAN AUTHOR ZERO YT7 ]\n\n', 'orange'), color('\n======TERIMAKASIH BANYAK KEPADA======\n•MHANKBARBAR\n•ZAKIR HORIZONTAL\n•KURRXD\n•NINO\n•IKYADS\n•KurrXd\n•DAPPAUHUY\n•DAN CREATOR BOT LAINNYA', 'yellow'))
	console.log(color('\n\nSARAN JANGAN DI JUAL ULANG BRO\nPERCAYA AJA\nNTAR NYESEL', 'pink'))
	console.log(color('\n\n[ WKWK NTAR DIKONTOLIN', 'red'))
	dha.browserDescription = ["Xyeenza CPH77", "Chrome", "3.0.0"];

	// Menunggu QR
	dha.on('qr', () => {
		console.log(color('[', 'pink'), color('!', 'red'), color(']', 'pink'), color('SCAN BRO...'))
	})

	// Menghubungkan
	fs.existsSync(`./${setting.sessionName}.json`) && dha.loadAuthInfo(`./${setting.sessionName}.json`)
	dha.on('connecting', () => {
		console.log(color('[ Xyeenza CPH77 ]', 'yellow'), color('PROSES NYAMBUNG...'));
	})
const spinner = { 
  "interval": 120,
  "frames": [
	...
	//connect
	dha.on('open', () => {
		console.log(color('[ Xyeenza CPH77 ]', 'yellow'), color('BOT AKTIF'));
	})
